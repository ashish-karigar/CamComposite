...
class _FrameDelegate(Foundation.NSObject):
    def init(self):
    ...
    def captureOutput_didOutputSampleBuffer_fromConnection_(self, output, sample_buffer, connection):
        ...
        try:
            ...
            if not base_address or width <= 0 or height <= 0 or bytes_per_row <= 0:
                return
...
class MacAVFoundationCapture:
    ...
    def open(self):
        ...
        self.device_input, error = AVFoundation.AVCaptureDeviceInput.deviceInputWithDevice_error_(
            device, None
        )
